  ...
  export class UserDashboardComponent implements OnInit, OnDestroy {
    getStartButtonTooltip(appointment: Appoitement): string {
      ...
      if (appointment.status === 'started' && meetingUrl) return 'Rejoindre la téléconsultation';
      if (appointment.status === 'started') return 'Téléconsultation démarrée (lien indisponible)';
      if (!this.isStatusValidated(appointment.status)) return 'Le rendez-vous doit être validé';
    ...
    joinZoomMeeting(appointment: Appoitement): void {
      ...
      if (!url) {
        this.showNotification('Aucun lien de téléconsultation disponible pour ce rendez-vous', 'error');
        return;
      }
